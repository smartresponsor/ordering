...
final class OrderE2EFlowTest extends WebTestCase
{
    ...
    public function testFullOrderPaymentRefundFlow(): void
    {
        ...
        // 2) Generate invoice (API): POST /api/order/invoice/{id}
        $client->request('POST', sprintf('/api/orders/%d/invoice', $order->getId()));
        self::assertResponseIsSuccessful();

        // 3) Create payment intent (API): POST /api/order/payment/{id}
        ...
        $client->request('POST', '/api/payment/webhook/refund', server: [
            'HTTP_CONTENT_TYPE' => 'application/json',
            'HTTP_X_PROVIDER' => 'mock',
