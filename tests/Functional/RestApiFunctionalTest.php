...
final class RestApiFunctionalTest extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        self::$kernel->boot();
        self::$client = new KernelBrowser(self::$kernel);
    }
    ...
    public function testCreatePayShipFlow(): void
    {
        ...
        // Get order
        self::$client->request('GET', "/orders/$orderId");
        $this->assertSame(200, self::$client->getResponse()->getStatusCode());
        $json = json_decode(self::$client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        $this->assertArrayHasKey('status', $json);
        $this->assertTrue(in_array($json['status'], ['paid', 'shipped', 'completed'], true));
    }
}
