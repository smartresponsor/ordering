...
final class CustomerOrderReadServiceTest extends TestCase
{
    public function testCustomerScopedProjectionAndLookup(): void
    {
        ...
        $order->addItem(new OrderItemEntity('SKU-RETURN-1', 2, '25.00', 'USD'));
        ...
        $details = $service->listDetailsForCustomer('customer-1');
        self::assertCount(1, $details);
        self::assertCount(1, $details[0]->items);
        self::assertSame('SKU-RETURN-1', $details[0]->items[0]->reference);
        self::assertSame(2, $details[0]->items[0]->quantity);
        self::assertSame('25.00', $details[0]->items[0]->unitPrice);
        self::assertNotNull($service->findDetailForCustomer('customer-1', 'ORD-CUSTOMER-1'));
        self::assertNull($service->findDetailForCustomer('customer-1', 'ORD-OTHER'));
    }
