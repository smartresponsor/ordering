...
final class OrderStatsCommand extends Command
{
    ...
    {
        $rows = $this->em->createQuery('SELECT o.status AS status, COUNT(o.id) AS qty FROM '.OrderEntity::class.' o GROUP BY o.status')->getArrayResult();
        $sum = (string) $this->em->createQuery('SELECT COALESCE(SUM(p.amount), 0) FROM '.OrderPaymentEntity::class.' p')->getSingleScalarResult();
        $count = (int) $this->em->createQuery('SELECT COUNT(o.id) FROM '.OrderEntity::class.' o')->getSingleScalarResult();
