...
- **Controller**: `PaymentWebhookController` с маршрутами:
  ...
  - `POST /api/payment/webhook/refund`
...
```bash
curl -X POST http://localhost:8000/api/payment/webhook   -H 'Content-Type: application/json'   -H 'X-Provider: mock'   -H 'X-Event-Id: evt_123'   -d '{"intent":"pi_abcdef123456","status":"succeeded","currency":"USD"}'
```
