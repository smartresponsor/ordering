# Ordering runtime boot audit вЂ” wave 28

## Findings addressed
- `config/packages/messenger.php` referenced `App\Ordering\Middleware\IdempotencyMiddleware` while the canonical implementation lives under `App\Messenger\Middleware\IdempotencyMiddleware`.
...
- Added a compatibility wrapper under `App\Ordering\Middleware\IdempotencyMiddleware`.
